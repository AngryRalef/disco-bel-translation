// Vendored from UABEANext (https://github.com/nesrak1/UABEANext), MIT License, Copyright (c) 2025 nesrak1.
// AssetsTools.NET.Texture/TextureFile.cs (+ this Texture/ subtree) - reused here so
// texture reimport goes through UABE's real encode logic (format handling, resizing,
// stream-data clearing) instead of hand-rolling it like the font atlas code does.
// The published AssetsTools.NET.Texture NuGet package (3.0.2) has drifted from this
// source's API (same class of version-skew already hit with AssetsTools.NET.Cpp2IL),
// so the source is vendored directly rather than referenced from NuGet.
